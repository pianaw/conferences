...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
    ...
    @Query("FROM Schedule schedule WHERE schedule.date = :date AND ((schedule.start <= :end AND schedule.end >= :end) OR (schedule.start >= :start AND schedule.end <= :end) OR (schedule.start <= :start AND schedule.end >= :start) OR (schedule.start >= :start AND schedule.end <= :end)) AND schedule.room.id = :roomId")
